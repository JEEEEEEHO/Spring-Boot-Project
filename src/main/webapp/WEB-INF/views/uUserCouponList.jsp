						...
						<ul class="clearfix">
							<c:forEach var="ucoupon" items = "${listUserCoupon }">
								<li>
										...
										<span class="text_box">
											<p>${ucoupon.coup_name }</p>
											<p><fmt:formatDate value="${ucoupon.coup_start}" pattern="yyyy.MM.dd" />
											~
											<fmt:formatDate value="${ucoupon.coup_end}" pattern="yyyy.MM.dd" /></p>
											<c:if test="${ucoupon.coup_type == 1 }">
											<h4>${ucoupon.coup_dc_rate}% 할인</h4>
											</c:if>
											<c:if test="${ucoupon.coup_type == 2 }">
											<h4>${ucoupon.coup_dc_price}원 할인</h4>
											</c:if>
										</span>
								</li>
							</c:forEach>
