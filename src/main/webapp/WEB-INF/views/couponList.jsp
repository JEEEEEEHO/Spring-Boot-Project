...
<body>
	...
	<c:forEach var="coupon" items="${coup_list }">
		<div>
			...
			<p><fmt:formatDate value="${coupon.coup_start }" type="DATE" pattern="yyyy-MM-dd"/> ~ 
			<fmt:formatDate value="${coupon.coup_end }" type="DATE" pattern="yyyy-MM-dd"/></p>
			<p><fmt:formatNumber value="${coupon.coup_req_price }" groupingUsed="true"></fmt:formatNumber> 이상 구매시 적용</p>
			...
			<button onclick="couponSelect(${coupon.coupno},${coupon.coup_type },
							 ${coupon.coup_req_price }, ${coupon.coup_dc_rate }, ${coupon.coup_dc_price })">선택</button>
		</div>
	</c:forEach>
	...
	<script type="text/javascript">
		function couponSelect(coupno, coup_type, coup_req_price, coup_dc_rate, coup_dc_price) {
			...
			var coup_sale = 0;
			if(total_amount < coup_req_price) {
				alert("최소 가격을 충족하지 못했습니다.");
			} else {
				if(coup_type == 1) {
					coup_sale = Math.ceil((total_amount*(coup_dc_rate / 100))/10) * 10;
				} else {
