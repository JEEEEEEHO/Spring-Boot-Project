...
public class JhPaymentDetailDaoImplU implements JhPaymentDetailDaoU {
	...
	public List<Payment> orderList(String id) { // 구매내역 회원 id에 따른 구매상태 조회 
		...
		try {
			...
			System.out.println("JhPaymentDetailDaoImplU orderList.get(0).getPayno()"+orderList.get(0).getPayno());
			System.out.println("JhPaymentDetailDaoImplU orderList.get(0).getPay_sts()"+orderList.get(0).getPay_sts());
		} catch (Exception e) {
