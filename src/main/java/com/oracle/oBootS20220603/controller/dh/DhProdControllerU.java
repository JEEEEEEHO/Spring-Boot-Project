...
//		model.addAttribute("memberId", "testuser");
	...
	public String prodDetail(Product product, HttpServletRequest request, Model model) { //model값안에 prodList와 그 외의 값 
	...
	// [아작스] 위시리스트 하트(빈->채) 바뀜
		...
		// [아작스] 위시리스트 하트(채->빈) 바뀜
			...
			public String wishList(HttpServletRequest request, Model model) {
				...
				System.out.println("DhProdControllerU idWishList idWishList.get(0).getProdno()->"+idWishList.get(0).getProdno());
