...
public class DhProdControllerA {
	...
	public String prodSelect(Product product, String currentPage, Model model) {
		logger.info("DhProdControllerA prodSelect Start..");
		int total = dhps.total();
	...
	public String prodView(int prodno, Model model) {
		logger.info("DhProdControllerA prodView Start..");
		
		Product product = dhps.prodDetail(prodno);
	...
	public String prodWrite(Product product, Model model, HttpServletRequest request,
			MultipartFile thimg, MultipartFile main1, MultipartFile main2) throws Exception {
		logger.info("DhProdControllerA prodWrite Start..");
		
		String uploadPath = request.getSession().getServletContext().getRealPath("/upload/");
		
		logger.info("originalName: " + thimg.getOriginalFilename());
		logger.info("size: " + thimg.getSize());
		logger.info("contentType: " + thimg.getContentType());
		logger.info("uploadPath: " + uploadPath);
		String savedName1 = uploadFile(thimg.getOriginalFilename(), thimg.getBytes(), uploadPath);		// 배너이미지 파일 저장
		String savedName2 = uploadFile(main1.getOriginalFilename(), main1.getBytes(), uploadPath);	// 대표이미지1 파일 저장
		String savedName3 = uploadFile(main2.getOriginalFilename(), main2.getBytes(), uploadPath);	// 대표이미지2 파일 저장
		System.out.println("savedName1->"+savedName1);
		System.out.println("savedName2->"+savedName2);
		System.out.println("savedName3->"+savedName3);
		
//						      바로 아래 uploadFile 메서드가 있음. 메서드를 따로 빼서 만들어 줌
		logger.info("thimg: ", savedName1);
		logger.info("main1: ", savedName2);
		logger.info("main2: ", savedName3);
		
		product.setTh_img(savedName1);
		product.setMain_img1(savedName2);
		product.setMain_img2(savedName3);
		
		model.addAttribute("th_img", savedName1);
		model.addAttribute("main_img1", savedName2);
		model.addAttribute("main_img2", savedName3);
		
		int result = dhps.prodInsert(product);
		if (result > 0)
			return "redirect:aDhProdList";
		else {
